...
def verify_and_replay_rtmr3(
    ...
    if count == 0:
        raise verification_failure(
            'measurement', 'measurement.rtmr3_mismatch', {'reason': 'no_events'}
        )
    ...
    if entry.event_type == DSTACK_RUNTIME_EVENT_TYPE:
        ...
        if entry.digest:
            ...
            if len(stored) != 48 or stored != computed:
                ...
                raise verification_failure(
                    'measurement', 'measurement.event_log_invalid', details
                )
        return computed
...
def _invalid_event(path: str, reason: str, *, expected: str | None = None):
    ...
    return verification_failure('measurement', 'measurement.event_log_invalid', details)
