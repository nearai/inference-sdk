...
async def test_external_openai_uses_configured_header_only_authentication(monkeypatch):
    gateway = Gateway()
    gateway.install(monkeypatch)
...
async def test_async_request_body_is_sent_with_buffered_framing(monkeypatch, e2ee):
    gateway = Gateway()
    gateway.install(monkeypatch)
